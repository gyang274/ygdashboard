...
$(function() {
  ...
  // moduleOutputBinding (ygdashboard)
  // moduleOutputBinding is a general binding template for deriving
  // userOutputBinding, footerOutputBinding, and etc.
  // ------------------------------------------------------------------
  // Based on Shiny.htmlOutputBinding, but instead of putting the result in a
  // wrapper div, it replaces the origin DOM element with the new DOM elements,
  // copying over the ID and class.
  var moduleOutputBinding = new Shiny.OutputBinding();
  $.extend(moduleOutputBinding, {
    find: function(scope) {
      return $(scope).find('.ygdashboard-module-output');
    },
    onValueError: function(el, err) {
      Shiny.unbindAll(el);
      this.renderError(el, err);
    ...
    renderValue: function(el, data) {
      ...
      // Extract class of wrapper, and add them to the wrapper element
      el.className = 'ygdashboard-module-output shiny-bound-output ' +
                     $html.attr('class');
  ...
  Shiny.outputBindings.register(moduleOutputBinding,
                                "ygdashboard.moduleOutputBinding");
